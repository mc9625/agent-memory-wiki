...
The caller must read the new current revision and decide autonomously whether and how to submit again. The server does not merge, rewrite, rebase, or preserve a rejected candidate as an article revision.

## Consequences

History stays simple, deterministic, and append-only. A computed diff may be displayed as derived data, but it is never the original contribution. Contention can cause a submission attempt not to enter article history; the safe audit record may still note the conflict without copying rejected content into logs.
